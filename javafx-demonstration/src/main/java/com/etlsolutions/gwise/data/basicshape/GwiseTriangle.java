...
public class GwiseTriangle extends GwisePolygon {
    ...
    public GwiseTriangle(GwiseTriangleBean bean) {
        super(bean);
    }
    ...
    public boolean isOnLeftOf(GwisePoint point) {
        return pointA.onLeftOf(point) && pointB.onLeftOf(point) && pointC.onLeftOf(point);
    }

    public boolean isOnRightOf(GwisePoint point) {
        return point.onLeftOf(pointA) && point.onLeftOf(pointB) && point.onLeftOf(pointC);
    }

    public boolean isOnTopOf(GwisePoint point) {
        return pointA.onTopOf(point) && pointB.onTopOf(point) && pointC.onTopOf(point);
    }

    public boolean isOnBottomOf(GwisePoint point) {
        return point.onTopOf(pointA) && point.onTopOf(pointB) && point.onTopOf(pointC);
    }
    ...
    public double area() {
        ...
        double s = p * (p - sideA.getLength())* (p - sideB.getLength())* (p - sideC.getLength());
    ...
    private double getPointToLineSegmentDistance(GwisePoint point, GwiseLine line) {
        ...
        double m2 = Double.min(point.distanceSquare(line.getStartPoint()), point.distanceSquare(line.getEndPoint()));
    ...
    public GwiseShapeType getType() {
        return GwiseShapeType.TRIANGLE;
    }
}
